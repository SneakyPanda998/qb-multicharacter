            ...
            if (data.toggle) {
                ...
                var DotsInterval = setInterval(function() {
                    ...
                    if (loadingProgress == 3) {
                        originalText = "Validere spillerdata"
                        $("#loading-text").html(originalText);
                    }
...
function setupCharInfo(cData) {
    if (cData == 'empty') {
        $('.character-info-valid').html('<span id="no-char">Det valgte karakter er ikke i bruk ennå.<br><br>Denne karkateren har ikke informasjon ennå.</span>');
    } else {
        var gender = "Man"
        ...
        '<div class="character-info-box"><span id="info-label">Kjønn: </span><span class="char-info-js">'+gender+'</span></div>' +
        '<div class="character-info-box"><span id="info-label">Nasjonalitet: </span><span class="char-info-js">'+cData.charinfo.nationality+'</span></div>' +
        '<div class="character-info-box"><span id="info-label">Jobb: </span><span class="char-info-js">'+cData.job.label+'</span></div>' +
        '<div class="character-info-box"><span id="info-label">Kontanter: </span><span class="char-info-js">&#36; '+cData.money.cash+'</span></div>' +
...
$(document).on('click', '.character', function(e) {
    ...
    if (selectedChar === null) {
        ...
        if ((selectedChar).data('cid') == "") {
            ...
            $(selectedChar).addClass("char-selected");
            setupCharInfo($(this).data('cData'))
            $("#play-text").html("Spill");
            $("#delete-text").html("Slett");
            $("#play").css({"display":"block"});
            if (EnableDeleteButton) {
                $("#delete").css({"display":"block"});
            }
...
$(document).on('click', '#create', function (e) {
    ...
    if(regTest.test(firstname) || regTest.test(lastname)){
        console.log("ERROR: Du brukte et nedsettende/vulgært begrep. Prøv igjen!")
        return false;
    }
...
function setCharactersList() {
    ...
    for (let i = 1; i <= NChar; i++) {
        htmlResult += '<div class="character" id="char-'+ i +'" data-cid=""><span id="slot-name">Ledig Plass<span id="cid"></span></span></div>'
    }
    htmlResult += '<div class="character-btn" id="play"><p id="play-text">Velg en karakter</p></div><div class="character-btn" id="delete"><p id="delete-text">Velg en karakter</p></div>'
